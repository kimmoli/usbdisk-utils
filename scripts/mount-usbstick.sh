...
# mkfs.vfat /dev/sda1 then repartitioning to create an empty ext2 partition
...
systemd-cat -t mount-usbstick /bin/echo "Called to ${ACTION} ${DEVNAME}"
...
if [ "$ACTION" = "add" ]; then
    ...
    if [ -n "$DIR" ]; then
        systemd-cat -t mount-usbstick /bin/echo "${DEVNAME} already mounted on ${DIR}, ignoring"
        exit 0
    fi

    # This hack is here to delay mounting the usbstick until tracker is ready
    ...
    while true; do 
        ...
        systemd-cat -t mount-usbstick /bin/echo "Waiting $count seconds for tracker"
        sleep $count ; 
        count=$(( count + count ))
    done
	...
	*)
    ...
    systemd-cat -t mount-usbstick /bin/echo "Finished ${ACTION}ing ${DEVNAME} of type ${TYPE} at $MNT/${UUID}"

else
    DIR=$(grep -w ${DEVNAME} /proc/mounts | cut -d \  -f 2)
    if [ -n "${DIR}" ] ; then
        if [ "${DIR##$MNT}" = "${DIR}" ]; then
            systemd-cat -t mount-usbstick /bin/echo "${DEVNAME} mountpoint ${DIR} is not under ${MNT}, ignoring"
            exit 0
        fi
        umount $DIR || umount -l $DIR
        systemd-cat -t mount-usbstick /bin/echo "Finished ${ACTION}ing ${DEVNAME} at ${DIR}"
    fi
fi
